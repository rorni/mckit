...
double cylinder_func(
  unsigned int n, const double* x, double* grad, void* f_data)
{
...
double gq_func(unsigned int n, const double* x, double* grad, void* f_data)
{
    ...
    if (grad != NULL) {
        ...
        cblas_dgemv(CblasRowMajor, CblasNoTrans, NDIM, NDIM, 2, data->m, NDIM,
          x, 1, 1, grad, 1);
    ...
    cblas_dgemv(
      CblasRowMajor, CblasNoTrans, NDIM, NDIM, 1, data->m, NDIM, x, 1, 1, y, 1);
...
void surface_test_points(
  const Surface* surf, size_t npts, const double* points, char* result)
{
    int i;
    double fval;
    for (i = 0; i < npts; ++i) {
...
int surface_test_box(Surface* surf, const Box* box)
{
    ...
    if (sign != 0 && surf->type != PLANE) {
        ...
        for (i = 0; i < NCOR; ++i) {
            ...
            if (sign * opt_val < 0) { // If sign and found opt_val have
