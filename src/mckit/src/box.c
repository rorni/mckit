 ...
 */
static double perm[NCOR][NDIM] = {{-1, -1, -1}, {-1, -1, 1}, {-1, 1, -1},
...
int box_init(Box* box, const double* center, const double* ex, const double* ey,
  const double* ez, double xdim, double ydim, double zdim)
{
    ...
    for (i = 0; i < NCOR; ++i) {
        ...
        cblas_daxpy(NDIM, 0.5 * perm[i][0] * box->dims[0], box->ex, 1,
          box->corners + i * NDIM, 1);
        cblas_daxpy(NDIM, 0.5 * perm[i][1] * box->dims[1], box->ey, 1,
          box->corners + i * NDIM, 1);
        cblas_daxpy(NDIM, 0.5 * perm[i][2] * box->dims[2], box->ez, 1,
          box->corners + i * NDIM, 1);
    }
...
{
    box_init(dst, src->center, src->ex, src->ey, src->ez, src->dims[0],
      src->dims[1], src->dims[2]);
...
int box_generate_random_points(Box* box, size_t npts, double* points)
{
    ...
    for (i = 0; i < npts; ++i) {
        status = vdRngUniform(
          VSL_RNG_METHOD_UNIFORM_STD, box->rng, NDIM, d, -0.5, 0.5);
        if (status != VSL_STATUS_OK)
            return BOX_FAILURE;

        cblas_dcopy(NDIM, box->center, 1, points + i * NDIM, 1);
        cblas_daxpy(
          NDIM, d[0] * box->dims[0], box->ex, 1, points + i * NDIM, 1);
        cblas_daxpy(
          NDIM, d[1] * box->dims[1], box->ey, 1, points + i * NDIM, 1);
        cblas_daxpy(
          NDIM, d[2] * box->dims[2], box->ez, 1, points + i * NDIM, 1);
    }
    return BOX_SUCCESS;
}

void box_test_points(
  const Box* box, size_t npts, const double* points, int* result)
{
    double delta[NDIM];
    double x, y, z;
    int i;
...
int box_split(const Box* box, Box* box1, Box* box2, int dir, double ratio)
{
    ...
    int status;
    status = box_init(
      box1, center1, box->ex, box->ey, box->ez, dims1[0], dims1[1], dims1[2]);
    if (status == BOX_FAILURE)
        return BOX_FAILURE;

    status = box_init(
      box2, center2, box->ex, box->ey, box->ez, dims2[0], dims2[1], dims2[2]);
    if (status == BOX_FAILURE)
        return BOX_FAILURE;
...
void box_ieqcons(unsigned int m, double* result, unsigned int n,
