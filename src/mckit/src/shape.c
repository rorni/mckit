 ...
 */
int shape_test_box(
  Shape* shape, const Box* box, char collect, int* zero_surfaces)
{
    if (shape->last_box != 0) {
        ...
        char use_cache =
          (bc > 0 && shape->last_box_result != BOX_CAN_INTERSECT_SHAPE);
    ...
    if (is_final(shape->opc)) {
        ...
        for (int i = 0; i < shape->alen; ++i) {
            sub[i] = shape_test_box(
              (shape->args.shapes)[i], box, collect, zero_surfaces);
        }
...
int set_zero_surface_pointers(
  Shape* shape, int n, Surface** zs, uint64_t subdiv)
{
    if (is_final(shape->opc)) {
        if (shape->args.surface->last_box == subdiv &&
            shape->args.surface->last_box_result == 0) {
            char already = 0;
            for (int i = 0; i < n; ++i) {
...
)
{
    ...
    if (result == BOX_CAN_INTERSECT_SHAPE && box->volume > min_vol) {
        ...
        if (result1 != BOX_CAN_INTERSECT_SHAPE &&
            result2 != BOX_CAN_INTERSECT_SHAPE)
...
)
{
    ...
    if (is_final(shape->opc)) {
        ...
        for (i = 0; i < n; ++i) {
            shape_test_points(
              (shape->args.shapes)[i], npts, points, sub + i * npts);
        }
        for (i = 0; i < npts; ++i)
