    ...
    def __init__(self, names):
        ...
        for e in names:
            for n in self._names:
                ...
                self._names.append(e)

    def add(self, name_case):
        if len(self._names) == 0:
            self._names.append(name_case)
            return
        l = len(name_case)
        i = 0
        while i < len(self._names) and l >= len(self._names[i]):
            if name_case.issuperset(self._names[i]):
                return
            i += 1
        self._names.insert(i, name_case)
        i += 1
        while i < len(self._names):
            if self._names[i].issuperset(name_case):
                self._names.pop(i)
            else:
                i += 1

    def combine(self, other):
        names = product(self._names, other._names)
        return ImportantNodes(names)
...
class GeometryNode:
    ...
    def __init__(self, operation, *args, node_id=None):
    ...
    def _clean_args(self, args):
        """Transforms args to simplier form."""
        if len(args) == 0:
            return args
        elif len(args) == 1:
            arg = args.pop()
        ...
        cleaned = set()
        for a in args:
            if a.is_of_type(self._opc):
                cleaned.update(a._args)
            else:
                cleaned.add(a)

        comp_set = {a.complement() for a in cleaned}
        overlap = comp_set.intersection(cleaned)
        if len(overlap) > 0:
    ...
    def to_additive_form(self):
        """Converts geometry tree to additive form - union of intersections."""
        if isinstance(self._args, Surface):
            return self
        else:
            args = {a._to_additive_form() for a in self._args}
            if self._opc == self._UNION:
                return GeometryNode('U', *args, node_id=self._id)
            else:
                unions = {a._args for a in args if a._opc == self._UNION}
                simple = {a for a in args if
                          a._opc == self._IDENTITY or a._opc == self._COMPLEMENT}
                simple.update(reduce(set.union, [a._args for a in args if
                                                 a._opc == self._INTERSECTION]))
                union_terms = product(*unions)
                args = [set(u).union(simple) for u in union_terms]
                return GeometryNode('U', *args, node_id=self._id)
